...
contains 
    ...
    subroutine Morlighem17_to_grid_cdo(outfldr,grid,domain,grad_lim)
        ...
        if (trim(domain) .eq. "Greenland") then 
            ...
            ! == Original dx=150m resolution ==
            ! Note: this file is very large and needs special care for memory limits
            ! file_in = "/data/sicopolis/data/Greenland/Morlighem2017_BedMachine/BedMachineGreenland-2017-09-20.nc"
            ! call grid_init(grid0,name="M17-150M",mtype="polar_stereographic", &
        ...
        call def_var_info(vars(3),trim(file_in),"thickness","H_ice",units="m",long_name="Ice thickness",    method="mean")
        call def_var_info(vars(4),trim(file_in),"errbed",   "z_bed_err",units="m",long_name="Bedrock / ice thickness error",method="mean")
        call def_var_info(vars(5),trim(file_in),"mask",     "mask",units="(0 - 3)", &
                          long_name="(0 = ocean, 1 = ice-free land, 2 = grounded ice, 3 = floating ice)",method="count")
