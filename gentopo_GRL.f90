...
!     character(len=256) :: file_input, file_output, file_test, var_name, file_topo
...
!     integer :: t, ttot
    ...
    file_ice       = "output/GRL-20KM_ERA-INTERIM-600Mb.nc"
    file_clim      = "output/GRL-50KM_ERA-INTERIM-600Mb.nc"
!     file_mar       = "data/MAR/"
...
!     call nc_read(file_topo,TOPO%H, "IceThickness",    missing_value=missing_value)
    ...
    call map_init(mECMWF_ice, gECMWF,gice, max_neighbors=10,lat_lim=4.d0,fldr="maps",load=.TRUE.)
    call map_init(mECMWF_clim,gECMWF,gclim,max_neighbors=10,lat_lim=4.d0,fldr="maps",load=.TRUE.)
    ...
    file_pres      = "data/ECMWF/NEW/ERA-INTERIM-GRL-600Mb_historical_mon_197901-201212.nc"
    ...
    call map_field(mECMWF_clim,var_now%nm_in,invar,climvar,climmask,"shepard",100.d3,missing_value=missing_value)
    call nc_write(file_clim,climvar,var_now%nm_out,  dim1="xc",dim2="yc",units=var_now%units_out)
    call map_field(mECMWF_ice, var_now%nm_in,invar,icevar, icemask, "shepard",100.d3,missing_value=missing_value)
...
contains 
    ...
    subroutine def_var_info(var,filename,nm_in,nm_out,units)
        implicit none 
